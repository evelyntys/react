...
export default class SurveyForm extends React.Component {
    ...
    updateFruits = (event) => {
        ...
        if (this.state.allFruits.includes(event.target.value)) {
            // how to remove from an array
            let indexToRemove = this.state.allFruits.indexOf(event.target.value);
            //spreading out the ones you want to remove, then joining everything together to become an array
            let cloned = [...this.state.allFruits.slice(0, indexToRemove), ...this.state.allFruits.slice(indexToRemove + 1)]
            this.setState({
                'fruits': cloned
            })
            ...
            let cloned = [...this.state.allFruits, event.target.value] //adding in new fruit at the back
